...
|---------|---------|-----------|
| Build Docker image (first time, or after `Dockerfile` changes) | `docker_image.bat` | `./docker_image.sh` |
| Build Pi1541 (syncs source automatically) | `docker_build.bat` | `./docker_build.sh` |
...
Run once on first use. Repeat only if the `Dockerfile` changes (e.g. toolchain update). Source changes do **not** require re-running this step.
...
Run every time you want to build, including after source changes. The script automatically syncs the current source into the image (Layer 5 only — fast, no downloads) before building. Optionally pass a target architecture.
