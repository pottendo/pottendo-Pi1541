...
bool DiskImage::WriteD81(char *name)
{
	const unsigned physicalSectors = 10;
...
static const unsigned char emptyD81[] = {
0x28,0x03,0x44,0x00,0xa0,0xa0,0xa0,0xa0,0xa0,0xa0,0xa0,0xa0,0xa0,0xa0,0xa0,0xa0,
0xa0,0xa0,0xa0,0xa0,0xa0,0xa0,0x30,0x30,0xa0,0x33,0x44,0xa0,0xa0,0x00,0x00,0x00,

0x28,0x02,0x44,0xbb,0x30,0x30,0xc0,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
...
0x00,0xff,0x44,0xbb,0x30,0x30,0xc0,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
0x28,0xff,0xff,0xff,0xff,0xff,0x28,0xff,0xff,0xff,0xff,0xff,0x28,0xff,0xff,0xff,
0xff,0xff,0x28,0xff,0xff,0xff,0xff,0xff,0x28,0xff,0xff,0xff,0xff,0xff,0x28,0xff,
0xff,0xff,0xff,0xff,0x28,0xff,0xff,0xff,0xff,0xff,0x28,0xff,0xff,0xff,0xff,0xff,
0x28,0xff,0xff,0xff,0xff,0xff,0x28,0xff,0xff,0xff,0xff,0xff,0x28,0xff,0xff,0xff,
...
unsigned DiskImage::CreateNewD81DiskInRAM(const char* filenameNew, const char* ID, unsigned char* destBuffer)
{
    if (destBuffer == nullptr)
        destBuffer = DiskImage::readBuffer;

    // Zero-fill entire image
    memset(destBuffer, 0, MAX_D81_SIZE);
	memcpy(destBuffer + 0x61800, emptyD81, 32); 
	...
	unsigned char* ptr = destBuffer + 0x61800 + 0x04;
	int len = strlen(fn) - 4; 				// remove extension .d81
	if (len > 16) len = 16;
	for (int i = 0; i < len; ++i)			// filename as name in D81 dir header
		*ptr++ = ascii2petscii(fn[i]);
	// no need for 0x0a padding as it's in the constant
	ptr = destBuffer + 0x61800 + 0x16;
	for (int i = 0; i < 2; ++i)
		*ptr++ = ascii2petscii(ID[i]);

	memcpy(destBuffer + 0x61900, emptyD81 + 32, sizeof(emptyD81) - 32); // BAM and directory
