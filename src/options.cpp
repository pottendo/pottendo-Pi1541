...
void Options::Process(char* buffer)
{
	...
	while ((pOption = GetToken()) != 0)
	{
		...
		ELSE_CHECK_DECIMAL_OPTION(disableHDMI)
		ELSE_CHECK_DECIMAL_OPTION(supportUARTInput)
		ELSE_CHECK_DECIMAL_OPTION(hdmiGraphIEC)
		ELSE_CHECK_DECIMAL_OPTION(hdmiDisplayIECActivity)
		ELSE_CHECK_DECIMAL_OPTION(displayTracks)
		ELSE_CHECK_DECIMAL_OPTION(quickBoot)
		ELSE_CHECK_DECIMAL_OPTION(logoDisplayDelay)
		...
		ELSE_CHECK_DECIMAL_OPTION(maxContentSize)
		ELSE_CHECK_DECIMAL_OPTION(maxMultipartSize)
