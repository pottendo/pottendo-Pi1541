# pottendo-Pi1541 - DualDrive branch
...
Use LST on drive 9 to make disk changes more robust. E.g. on GEOS.
...
| Drive0DeviceID | 8, 9, 10, 11 | Selects device ID for drive 0, same as deviceID, if negative, e.g. -8, drive is not switched on automatically | 8 | no sanity check done |
| Drive1DeviceID | 8, 9, 10, 11 | Selects device ID for drive 1, if negative, e.g. -9, drive is not switched on automatically | -9 | no sanity check done|
